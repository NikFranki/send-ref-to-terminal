...
class FileRefTerminalLinkProvider implements vscode.TerminalLinkProvider<
  vscode.TerminalLink & { data: FileRefLinkData }
> {
  ...
  ): Promise<Array<vscode.TerminalLink & { data: FileRefLinkData }>> {
  ...
  handleTerminalLink(
    link: vscode.TerminalLink & { data: FileRefLinkData },
  ): void {
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.window.registerTerminalLinkProvider(
      new FileRefTerminalLinkProvider(),
    ),
  );

  const fileCmd = vscode.commands.registerCommand(
    "sendRefToTerminal.sendFile",
    (uri: vscode.Uri) => {
  ...
  const folderCmd = vscode.commands.registerCommand(
    "sendRefToTerminal.sendFolder",
    (uri: vscode.Uri) => {
      if (!uri) return;
      const ref = getRelativePath(uri.fsPath);
      const terminal =
        vscode.window.activeTerminal ?? vscode.window.createTerminal();
      terminal.show(false);
      terminal.sendText(ref, false);
  ...
  context.subscriptions.push(folderCmd);

  context.subscriptions.push(
    vscode.languages.registerCodeActionsProvider(
      "*",
      new AddToTerminalActionProvider(),
    ),
  );
